...
const HistoryPage: React.FC = () => {
  ...
  if (loading) {
    return <div className="flex items-center justify-center"> <Loader2Icon className="animate-spin"/> </div>;  // Display loading indicator
  }
        ...
        <tbody>
          {history.map((item) => (
            <tr key={item.id}>
              ...
              <td className="border border-gray-300 px-4 py-2 text-sm overflow-hidden" title={item.aiResponse}>{truncateResponse(item.aiResponse)} </td>
